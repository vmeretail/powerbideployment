function DisplayInstructions
{
	...
	Write-Host
	Write-Host "SETUP SCHEDULED TASKS FOR Cash Rec Reporting" -ForegroundColor yellow
	Write-Host "--------------------------------------" -ForegroundColor yellow
	Write-Host
...
function CreateScheduledTask($ScriptPath, $DBInstance, $DBName, $DBUserName, $DBPassword, $runType){
	...
	if ($runType -eq 0 )
    {
	    ...
	    $TaskName = "CashRecReporting-Daily" + $Organisation
        $Trigger= New-ScheduledTaskTrigger -At $DailyTaskRunTime -Daily
    }
    elseif($runType -eq 1) {
        # Every 15 mins
        $TaskName = "CashRecReporting-Every15Mins" + $Organisation
...
try {
	...
	do {
		...
		$ScriptPath = Join-Path $ScriptLocation "CashRecReport.ps1"
